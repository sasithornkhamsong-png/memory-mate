...
public class ShowStat_Game2 : MonoBehaviour
{
    ...
    void DisplayStats()
    {
        ...
        List<Score2Entry> topScores = scoreGame2Manager.GetTopScores();
    ...
    // สำหรับลำดับ 1-3
    void SetSeparateText(int index, TextMeshProUGUI sText, TextMeshProUGUI tText, List<Score2Entry> list)
    {
        if (sText == null || tText == null) return;

        if (index < list.Count)
        {
            Score2Entry s = list[index];
    ...
    // สำหรับลำดับ 4-5
    void SetCombinedText(int index, TextMeshProUGUI cText, List<Score2Entry> list)
    {
