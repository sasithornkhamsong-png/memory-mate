...
public class ShowStat_Game3 : MonoBehaviour
{
    ...
    void DisplayStats()
    {
        ...
        List<Score3Entry> topScores = scoreGame3Manager.GetTopScores();
    ...
    // สำหรับลำดับ 1-3
    void SetSeparateText(int index, TextMeshProUGUI sText, TextMeshProUGUI tText, List<Score3Entry> list)
    {
        if (sText == null || tText == null) return;

        if (index < list.Count)
        {
            Score3Entry s = list[index];
    ...
    // สำหรับลำดับ 4-5
    void SetCombinedText(int index, TextMeshProUGUI cText, List<Score3Entry> list)
    {
